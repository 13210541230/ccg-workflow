...
- Lead 不直接修改源码；Critical 问题由 Lead 组织修复，并交由 Builder 或 `execute-worker` 落地。
...
2. **多模型审查（PARALLEL）**
   ...
   - **会话复用**：若步骤 1 获取到 `CODEX_PLAN_SESSION` / `CODEX_B_PLAN_SESSION`，分别在下面对应命令的 `--sandbox read-only` 后追加 `--SESSION_ID <...>`。
   ...
   **FIRST Bash call (Codex)**（若有 SESSION_ID 则 --SESSION_ID，否则新会话）:
   ```
   Bash({
     command: "python \"<BRIDGE>\" --cd \"{{WORKDIR}}\" --role \"<PLUGIN_ROOT>/prompts/codex/reviewer.md\" --sandbox read-only --PROMPT '审查以下变更：\n<git diff 输出或变更文件列表>\nOUTPUT (JSON):\n{\n  \"findings\": [\n    {\n      \"severity\": \"Critical|Warning|Info\",\n      \"dimension\": \"logic|security|performance|error_handling\",\n      \"file\": \"path/to/file\",\n      \"line\": 42,\n      \"description\": \"问题描述\",\n      \"fix_suggestion\": \"修复建议\"\n    }\n  ],\n  \"passed_checks\": [\"已验证的检查项\"],\n  \"summary\": \"总体评估\"\n}'",
   ...
   **SECOND Bash call (Codex) - IN THE SAME MESSAGE**（若有 SESSION_ID 则 --SESSION_ID，否则新会话）:
   ```
   Bash({
   ...
   **输出丢失检测**（⚠️ 必须执行）：
   ...
   - 若输出为空但 `exit_code: 0`：先用 `Read` 工具读取输出文件（路径在启动时的 `Output is being written to:` 中，使用 Windows 绝对路径格式）。若临时文件已清理，用 `Glob` 查找 `~/.claude/.ccg/outputs/*.txt` 读取最新文件。若仍无，用相同命令重新调用（`--SESSION_ID` 复用会话）。
...
5. **决策门**
   - **Critical > 0**:
     ...
     * 选择修复 → Lead 整理修复要求并派发给 Builder 或 `execute-worker`（参考 Codex 建议）
     * 修复后重新运行受影响的审查维度
     * 重复直到 Critical = 0
   - **Critical = 0**:
     * 报告通过，建议提交代码
