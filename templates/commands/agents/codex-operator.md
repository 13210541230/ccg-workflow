...
| 批准/回滚/追加指令 | **你（Agent）** | git checkout 回滚，SESSION_ID 追加 |
...
保存 `PLUGIN_ROOT` 和 `BRIDGE` 路径。若 `BRIDGE MISSING` → 终止。
...
```
Bash({
  command: "python \"<BRIDGE>\" --cd \"$(pwd)\" --role \"<PLUGIN_ROOT>/prompts/codex/<role>.md\" --yolo --PROMPT '<像对 Claude Code 说话一样的自然语言指令。项目上下文：<关键文件路径和技术栈信息>。约束：<不可修改的文件、编码规范等>'",
...
```
Bash({
  command: "python \"<BRIDGE>\" --cd \"$(pwd)\" --SESSION_ID <SESSION_ID> --yolo --PROMPT '<追加指令，如同人类的后续对话>'",
  run_in_background: true,
  timeout: 3600000,
...
```
Bash({
  command: "python \"<BRIDGE>\" --cd \"$(pwd)\" --SESSION_ID <SESSION_ID> --yolo --PROMPT 'src/problematic-file.ts 的修改有以下问题：1. <具体问题> 2. <具体问题> 请重新修改这个文件，注意：<修正要求>'",
  run_in_background: true,
  timeout: 3600000,
