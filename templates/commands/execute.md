...
**3.5 计划批判性审查**：
- 检查所有引用文件是否实际存在（Glob 验证）
- 验证行号范围准确性（如计划指定了行号）
- 消除歧义（如任务描述不清，停止并请求澄清）
...
在 Phase 3（原型获取）或 Phase 4（编码实施）遇到以下情况时，**立即停止**，不要猜测：
- Codex 返回空输出或明显截断
- Codex 返回的 diff 无法干净应用（冲突超过 2 处）
- 计划中有步骤描述模糊，有 2+ 种合理解释
- 发现计划范围外的关键依赖缺失
...
验证规则：
- 阅读每个命令的完整输出（不跳过，不假设成功）
- 不得基于部分输出声称任务完成
- 未获得新鲜证据前，禁止进入 Phase 5

**批次执行协议**：
- 每批 3 个步骤，执行后报告结果
- 每批完成后：列出完成项、列出失败项、确认是否继续

**阻塞协议**：
- 立即停止条件：文件不存在、依赖缺失、需求不明确、测试失败超过 2 次
- 停止时输出：`{ "status": "blocked", "reason": "...", "needs": "..." }`
...
**审计循环上限**：Phase 5.1-5.2 最多循环 2 轮。若 2 轮后仍存在 Critical 问题：
- 写入 `artifacts/audit-unresolved.md`（问题列表 + 已尝试修复 + 风险评估）
- 向用户展示并询问：继续修复 / 接受风险交付 / 回滚
