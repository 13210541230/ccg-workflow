...
**强制判定**：
- 若计划只是把已明确的小改动排成 1-3 步，**不要**额外调用 Codex。
- 若计划需要文件拆分、模块边界设计、顺序编排、风险缓解或跨模块落地，**必须**调用 Codex 双模型规划。
- **一旦已进入 Codex 双模型规划模式，禁止因超时/空输出而改成你自己补全计划。** 只允许重试或报告阻塞。
...
超时/失败恢复（必须执行，禁止静默降级）：
- 第 1 次失败：收窄输入范围，改用 `--prompt-file` 重试失败的 Codex 调用
- 第 2 次失败：新建会话再试一次，并记录原会话失败原因
- 连续 2 次仍失败：输出 `Codex blocked`，只汇报已确认的客观约束与缺失信息；不得伪装成完整实施计划
